...
- **Ali-OSS云端自动备份与计算**: 服务端接入了 `ali-oss`，针对 AI 生图请求等素材会在后台先经由 `sharp` 库格式化为最大 3072 像素宽的高质量 `.webp` 以大幅节约空间再执行上云；底层搭载了一套 CSV 双向时间戳比对引擎 (`oss_uploader.js`) 可以随时感知并互相同步多设备之间的 OSS 图床记录索引表。

### 4.4 图像审阅、裁切与扩图底层实现引擎
平台在 `Grid.tsx` 内构建了独家的高级空间渲染层以实现深度的图像操纵能力：
- **重构版图像审阅机制 (Annotation)**: `ZoomableImage` 组件封装了一个独立于底层的涂鸦审阅矩阵。用户点击图像时的相对连续 `X/Y` 坐标系会被立刻侦测汇算，并挂载为可视化的图钉标记。其背后衍生的审批状态以及反馈跟帖将直接合并封存回相关的表格记录参数中，实现所见即所得。
- **裁切数学逻辑**: 常规的裁切依赖于 CSS 蒙版边界。引擎在用户拖拽时会运算出异常精准的位移增量，并在原始图像边界 (`maxPosX/Y`) 层面对其进行严格限幅计算，从数学层面上彻底拒绝将图像强扯出界面、避免暴露出其后的负空间背景层。在保存时，引擎会将可视呈现的截面投射至临时的 HTML `<canvas>` 内生成确切的 Base64 媒体串。
- **扩图强制接管 (Outpaint Bypass)**: 一旦 `isOutpaintMode` 布尔值被主动设定为 true 时，上述的物理限幅逻辑将遭到刻意地停用阻断。引擎此时将会允许拖拽矢量坐标越过图像的正常边际系，并通过直接绘制黑屏背景 (`ctx.fillRect(0, 0, targetW, targetH)`) 来填充当前 Canvas 上的新暴露出的留白领地。经此打包得到的附带“大面积留黑负空间底图” 可以作为核心引导模版，直接推流至上游关联的 AI-Generative Stable Diffusion 节点中进行补全推算。
